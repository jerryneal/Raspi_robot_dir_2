...
void loop()
{
 ...
 {
   data = i;
   sendToShift(datapin, clockpin, latchpin, data, debug);
   delay(delayTime);   
 } */
 
 /*
 sendToShift(datapin, clockpin, latchpin, 1, debug);
 delay(delayTime);
 sendToShift(datapin, clockpin, latchpin, 0, debug);
 delay(delayTime);
 */
}

/* use shift register connected to pins## dataPin, clockPin and latchPin 
   to send out a byte of data.
   debug mode requires open serial connection and outputs data to terminal
*/
void sendToShift(int dataPin, int clockPin, int latchPin, byte data, boolean dMode)
{
  if (dMode)  {
    Serial.print("Data: ");
